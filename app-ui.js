...
window.router = {
    async navigate(page) {
        // ১. ইউটিউবের মতো হ্যাশ আপডেট (যাতে রিফ্রেশ করলে ব্রাউজার পুরো পেজ নতুন করে রিলোড না করে)
        ...
        // ২. লোডার দেখানো (শুধু কন্টেন্ট এরিয়ার জন্য)
        if(loader) {
        ...
        try {
            // পেজ লোড করার সময় ক্যাশ এড়াতে টাইমস্ট্যাম্প ব্যবহার
            const response = await fetch(`${page}.html?v=${Date.now()}`);
            if (!response.ok) throw new Error('Page not found');
            const html = await response.text();
            
            // ৩. হেডার-ফুটার স্থির রেখে শুধু মাঝখানের বডি পরিবর্তন করা
            ...
            // ৪. নেভিগেশন বাটনের একটিভ স্টেট আপডেট করা (HTML-এর data-route অনুযায়ী)
...
// 🔥 রিফ্রেশ সমস্যা সমাধানের মেইন লজিক
...
onAuthStateChanged(window.auth, user => {
    if (user) {
        ...
        // লগইন থাকার পর রিফ্রেশ দিলে URL হ্যাশ দেখে সেই বডি লোড করবে
        if (isInitialLoad) {
...
// ব্রাউজারের ব্যাক বাটন বা হ্যাশ চেঞ্জ হ্যান্ডেল করা (ইউটিউব স্টাইল)
